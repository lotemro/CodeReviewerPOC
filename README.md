...
The `tests/scripts/` directory contains tools for manual verification:
- `tests/scripts/parallelism_test.py`: Verifies concurrency limits by sending 10 simultaneous requests.
- `tests/scripts/bulk_runner.py`: Submits all Python files in `tests/scripts/sample_files` for review in batches.
